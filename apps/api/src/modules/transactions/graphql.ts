...
export function createFinanceGraphQL(auth: AuthService, transactions: TransactionService, categorization: CategorizationService, planning: PlanningService) {
  const schema = createSchema({
    typeDefs: `
      ...
      type Query { transactionReviewQueue(cursor: String, limit: Int, status: String): TransactionConnection!, financialDashboard(month: String!, currency: String!): FinancialDashboard! }
    ...
    resolvers: {
      ...
      Query: { transactionReviewQueue: (_root, args: { cursor?: string; limit?: number; status?: string }, context: { userId: string }) => transactions.reviewQueue(context.userId, args), financialDashboard: (_root, args: { month: string; currency: string }, context: { userId: string }) => planning.dashboard(context.userId, args) },
      Mutation: {
