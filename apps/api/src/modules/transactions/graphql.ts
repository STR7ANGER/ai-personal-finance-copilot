...
import { FinanceQaError, type FinanceQaService } from "../qa/service.js";
...
export function createFinanceGraphQL(auth: AuthService, transactions: TransactionService, categorization: CategorizationService, planning: PlanningService, forecasting: ForecastService, qa: FinanceQaService) {
  const schema = createSchema({
    typeDefs: `
      ...
      type FinanceFact { id: ID!, label: String!, sourceType: String!, sourceId: ID, valueMinor: String, text: String }
      type FinanceAnswer { id: ID!, question: String!, answer: String!, currency: String!, periodStart: String!, model: String!, promptVersion: String!, createdAt: String!, citations: [FinanceFact!]! }
      type Query { transactionReviewQueue(cursor: String, limit: Int, status: String): TransactionConnection!, financialDashboard(month: String!, currency: String!): FinancialDashboard!, cashFlowForecast(startingBalanceMinor: String!, horizonDays: Int!, currency: String!, incomeAdjustmentBps: Int, expenseAdjustmentBps: Int, events: [ScenarioEventInput!]): CashFlowForecast!, financeFacts(month: String!, currency: String!): [FinanceFact!]!, financeAnswer(id: ID!): FinanceAnswer! }
      type Mutation { setTransactionCategory(transactionId: ID!, categoryId: ID!, expectedVersion: Int!): Transaction!, resolveDuplicate(candidateId: ID!, resolution: String!, canonicalId: ID): Boolean!, suggestTransactionCategory(transactionId: ID!): CategorySuggestion!, recordCategoryFeedback(suggestionId: ID!, accepted: Boolean!, categoryId: ID, reason: String): Boolean!, createBudget(name: String!, month: String!, currency: String!, totalLimitMinor: String!): Budget!, createFinancialGoal(name: String!, targetMinor: String!, currentMinor: String, currency: String!, targetDate: String): FinancialGoal!, createSubscription(merchant: String!, amountMinor: String!, currency: String!, cadence: String!, nextChargeDate: String!): Subscription!, askFinance(question: String!, month: String!, currency: String!): FinanceAnswer! }
    `,
    resolvers: {
