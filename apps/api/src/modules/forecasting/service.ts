...
export type ForecastTelemetry = { record(event: { name: "forecast.generated"; durationMs: number; horizonDays: number; historyDays: number; pointCount: number }): void };

export class ForecastService {
  constructor(private readonly repository: ForecastRepository, private readonly clock = () => new Date(), private readonly telemetry: ForecastTelemetry = { record() {} }) {}
  async generate(userId: string, input: { startingBalanceMinor: string; horizonDays: number; currency: string; incomeAdjustmentBps?: number; expenseAdjustmentBps?: number; events?: Array<{ date: string; amountMinor: string; label: string }> }) {
    ...
    const result = forecast({ now, startingBalanceMinor: BigInt(input.startingBalanceMinor), horizonDays: input.horizonDays, currency, history: source.history, subscriptions: source.subscriptions, events, incomeAdjustmentBps, expenseAdjustmentBps });
    this.telemetry.record({ name: "forecast.generated", durationMs: Math.round(performance.now() - startedAt), horizonDays: input.horizonDays, historyDays: result.historyDays, pointCount: result.points.length });
    return result;
  }
}
